...
const allowGlobal = createRateLimiter(200, 60_000);
...
export async function POST(req: Request): Promise<Response> {
  ...
  if (hostName.length > 80) return NextResponse.json({ error: "hostName is too long." }, { status: 400 });

  if (!Array.isArray(body.options) || body.options.length < 2) {
    return NextResponse.json({ error: "At least 2 options are required." }, { status: 400 });
  }
  if (body.options.length > 50) {
    return NextResponse.json({ error: "Too many options (max 50)." }, { status: 400 });
  }
  ...
  for (const raw of body.options) {
    ...
    if (name.length > 200) return NextResponse.json({ error: "An option name is too long." }, { status: 400 });

    const placeId = (raw as { placeId?: unknown }).placeId;
    if (typeof placeId === "string" && placeId.length > 256) {
      return NextResponse.json({ error: "placeId is too long." }, { status: 400 });
    }

    const snapshot = (raw as { snapshot?: unknown }).snapshot;
    if (snapshot != null && JSON.stringify(snapshot).length > 4096) {
      return NextResponse.json({ error: "snapshot is too large." }, { status: 400 });
    }
